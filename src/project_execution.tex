...
\caption{Diagram of the Iterative Development Model.  \textit{\small Westerhoff [Public domain], via Wikimedia Commons}}
\label{fig:iter}
\end{figure}
...
Because this project is aiming to build a product that most closely matches the needs and wants of local recreational climbers, I decided to use the User-Centred design approach, which states ``users should be involved throughout the project development
...
In this first large iteration, I gathered initial thoughts and requirements from climbers via a survey, and then trialled some of these in a series of wizard-of-oz tests to glean the probable requirements of the users.
...
\subsection{Wizard-Of-Oz Prototypes and Tests}
To determine the requirement for this video feature vs using an accelerometer to provide data input, a series of informal discussions and wizard-of-oz (WOZ) tests were performed at a climbing wall with 3 boulderers.
...
Now I had completed this first iteration, and roughly knew which features and form-factors the users wanted, I had to design and implement a prototype.

\subsection{Fundamental Early Decisions for Development}
% Now I had completed the planning and requirements stages of the iterative design process, and roughly knew which features and form-factors the users wanted, I had to design and implement a prototype.
In order to do this, I had to make some choices regarding the tools I'd be using.
...
\subsection{First app: Recorder}
After the success of the accelerometer idea in the previous iteration's tests, I decided to build a real app based around this, to use in some real in-field testing.
...
Now that the app was relatively stable and the two big features (acceleration and video) were working well together, it was time to clean up the code, implement the list of small requests that had been piling up, and then deploy the app, publishing it on the Play Store.
